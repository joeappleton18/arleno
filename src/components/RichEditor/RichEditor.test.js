...
describe('RichEditor', () => {
    ...
    test('does not submit when empty', () => {
        const handleSubmit = jest.fn();
        render(<RichEditor onSubmit={handleSubmit} />);
        expect(screen.getByText(/Enter your answer here... 😊/i)).toBeInTheDocument();
        const submitButton = screen.getByText(/submit answer/i);
        fireEvent.click(submitButton);
        expect(handleSubmit).toHaveBeenCalledTimes(0);
    });

    test('submits when value in text box', () => {
        const handleSubmit = jest.fn();
        render(<RichEditor data={sample} onSubmit={handleSubmit} />);
        const submitButton = screen.getByText(/submit answer/i);
        expect(screen.getByText(/My Title/i)).toBeInTheDocument();
        expect(screen.queryByText(/Enter your answer here... 😊/i)).not.toBeInTheDocument();
        fireEvent.click(submitButton);
        expect(handleSubmit).toHaveBeenCalledTimes(1);
    });

    test('fires a cancel event when the button pressed', () => {
        const handleCancel = jest.fn();
        render(<RichEditor data={sample} onCancel={handleCancel} />);
