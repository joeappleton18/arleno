...
<MuiAlert severity="info">
  <AlertTitle>
    <strong> A Key Point </strong>
  </AlertTitle>
  This module is not about taking an in-depth exploration into developing web applications.
  Rather, I want to show you the basics to allow you to expose an interface for users
...
My rationale for taking the approach is a common technique in taking a NoSql approach to application development is to allow the intended user interactions with our application to guide the data model. In other words, we structure our data such that it is convenient for our future uses. As such, this week we are going to laying out a sample application and consider how our users will interact with it.
...
- We can now install exspress, `npm install express --save`
...
Next, type in the sample application , to run the application execute the following terminal command `npm run dev`.
...
Your main task for this week is to further explore the dataset and mock up a 4 page website using ejs templates that could be used to present the wine data in a meaningful way. **Note**, at this point we are not processing any data. Rather, we are mocking up what the data could look like. [You can use my example site as a starting point](example-website.zip). Notice how I am using Twitter Bootstrap to quickly generate a site. This is something you are more than welcome to do for your assessment. [You can read more about Bootstrap on the official site](https://getbootstrap.com/).
