...
<AnswerQuestionDialog question={"Within your groups, choose a single project idea. Next, briefly discuss, then nominate a team member top state, what the project is and where we may need to use each type of the three relationships"} id={questionIDs.find(q => q.count === 4).id}>
It's clear that the answer of what type of relationship we used is: **it depends**?. Let's consider this question for the group discussion today, along with the article on [relations](https://www.mongodb.com/blog/post/6-rules-of-thumb-for-mongodb-schema-design-part-1).
